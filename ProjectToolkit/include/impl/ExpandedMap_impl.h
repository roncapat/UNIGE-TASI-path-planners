...
ExpandedMap<E, I>::find_or_init(const ElemType &n) {
    ...
    assert(idx >= 0 and idx<buckets.size());
...
std::pair<float, float> ExpandedMap<E, I>::getGandRHS(const ElemType &s) const {
    auto idx = get_bucket_idx(s);
    if (idx ==-1 or idx >= buckets.size()) return {INFINITY, INFINITY};
    auto it = buckets[idx].find(s);
    if (it != buckets[idx].end())
        return {G(it), RHS(it)};
    else
...
float ExpandedMap<E, I>::getG(const ElemType &s) const {
    auto idx = get_bucket_idx(s);
    if (idx ==-1 or idx >= buckets.size()) return INFINITY;
    auto it = buckets[idx].find(s);
    if (it != buckets[idx].end())
        return G(it);
    else
...
std::optional<typename ExpandedMap<E, I>::iterator> ExpandedMap<E, I>::find(const E &n){
    auto idx = get_bucket_idx(n);
    if (idx ==-1 or idx >= buckets.size()) return {};
