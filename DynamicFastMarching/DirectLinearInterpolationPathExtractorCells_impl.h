...
template<typename T>
DirectLinearInterpolationPathExtractorCells<T>::DirectLinearInterpolationPathExtractorCells(
        const ExpandedMap<Cell,T> &map, const Graph & grid) : map(map), grid(grid){}

template<typename T>
void DirectLinearInterpolationPathExtractorCells<T>::extract_path() {
...
template<typename T>
bool DirectLinearInterpolationPathExtractorCells<T>::goalReached(const Position &p) {
