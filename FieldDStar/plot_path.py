...
def plot_path_on_map(img, prevpath=[], nextpath=[], expanded=[], info=None):
    ...
    if info is not None:
        ...
        fontpath = "./FieldDStar/SourceCodePro-Regular.ttf"
        font = ImageFont.truetype(fontpath, 64)
        out_map = cv2.copyMakeBorder(out_map, 150, 0, 0, 0, cv2.BORDER_CONSTANT, value=(255, 255, 255))
        img_pil = Image.fromarray(out_map)
        draw = ImageDraw.Draw(img_pil)
        draw.text((50, 30), "Field D*", font=font, fill=(100, 100, 100))
        draw.text((700, 10), caption1, font=font, fill=(100, 100, 100))
        draw.text((700, 60), caption2, font=font, fill=(100, 100, 100))
